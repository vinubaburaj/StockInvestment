...
public class ControllerFlexible {
  ...
  public void start(PortfolioFlexibleInterface portfolio) throws IOException {
    ...
    if (menuChoice == 1) {
      ...
      if (UtilityClass.checkFileExists(portfolioName, true)) {
        ...
        try {
          portfolio.createPortfolio(stocks, creationDate);
      ...
      try {
        ...
        out.append(view.showGenericMessage("Something went wrong while updating strategies."));
      }
      boolean run = true;
      while (run) {
        ...
        while (!UtilityClass.checkValidNumberOption(stringChoice, 1,
                9)) {
          out.append(view.displayErrorMessage("Invalid entry. Please choose "
        ...
        switch (choice) {
          ...
          case 4: {
            examinePortfolioController(portfolio);
            break;
          }
      ...
      out.append(view.showGenericMessage("Invalid Menu Choice entered"));
    }
  }
  ...
  private void examinePortfolioController(PortfolioFlexibleInterface portfolio)
          throws IOException {
    HashMap<String, Double> stocks = portfolio.examinePortfolio();
  ...
  private void purchaseSharesController(PortfolioFlexibleInterface portfolio, String portfolioName)
    ...
    if (stocksAppended.size() > 0) {
      boolean purchase = portfolio.purchaseStocks(stocksAppended);
      if (purchase) {
          ...
          if (isNum == 1) {
            ...
            if (UtilityClass.checkDateChronology(stringStockChoice, stringDate,
              ...
              if (UtilityClass.checkSellIsValid(portfolioName,
                ...
                sell = portfolio.sellStocks(stringStockChoice,
                        Integer.parseInt(stringNumberOfShares), stringDate);
              } else {
  ...
  private void findCostBasis(PortfolioFlexibleInterface portfolio, String portfolioName)
    ...
    if (UtilityClass.checkDateBeforeCreatedDate(portfolioName, stringDate) ||
      ...
      try {
        ...
        out.append(view.showGenericMessage("Something went wrong, please try again."));
      }
    }
  }
  ...
  private void getTotalValueFlexiblePortfolio(PortfolioFlexibleInterface portfolio,
    ...
    totalValue = portfolio.getTotalValue(stringDate);
  ...
  private void createInvestmentStrategy(PortfolioFlexibleInterface portfolio,
    ...
    startingDate = scan.next();
    while (!UtilityClass.checkDateFormat(startingDate)
            || UtilityClass.checkDateBeforeCreatedDate(portfolio.getPortfolioName(),
            startingDate)) {
      out.append("\nEnter valid date format in YYYY-MM-DD format. " +
              "Start date must be later than portfolio creation date\n");
      startingDate = scan.next();
    }
    ...
    while (!UtilityClass.checkDateFormatWithQuitter(endingDate, "none")
            || (!endingDate.equalsIgnoreCase("none")
            && LocalDate.parse(startingDate).isAfter(LocalDate.parse(endingDate)))) {
      out.append("\nEnter valid date format in YYYY-MM-DD format or enter 'none'. " +
              "Ending date should be after start date.\n");
      endingDate = scan.next();
    }
    ...
    while (UtilityClass.checkIfPositiveInteger(stringDays, "quit") != 1) {
      out.append(view.showGenericMessage("\nEnter valid positive number for days " +
              "(if you want to " +
              "add DCA only for the start-date, enter any positive number here):\n"));
      stringDays = scan.next();
    }
    days = Integer.parseInt(stringDays);
    try {
      portfolio.createStrategy(investAmount, stockList, startingDate,
              endingDate, days, strategyName);
      portfolio.updateStrategy();
    } catch (Exception e) {
      out.append((view.showGenericMessage("Something went wrong while executing strategies!")));
    }
  ...
  private HashMap<String, Integer> generateListOfStocksForInvestmentStrategy() throws IOException {
    ...
    while (run) {
      ...
      if (choice == 1) {
        ...
        while (!UtilityClass.checkValidNumberOption(stringPercentage,
                1, 100)) {
