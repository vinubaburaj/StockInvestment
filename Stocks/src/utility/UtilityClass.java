...
public final class UtilityClass {
  ...
  /**
   * This function to round double by definite places.
   *
   * @param value  the double value
   * @param places the number of digits after decimal.
   */
  ...
  /**
   * This function checks if the date is in correct format or if it is quitter.
   *
   * @param date    the date to check.
   * @param quitter the quitter to check.
   */
  ...
  /**
   * This function checks if the investment details are valid.
   *
   ...
   * @param recurringDays    the frequency.
   * @param pName            the portfolio name.
   */
  public static String checkValidInvestment(String strategyName, String investmentAmount,
                                            String startingDate, String endingDate,
                                            String recurringDays, String pName) {
    if (checkFileExists(pName + "_DCA_" + strategyName, true)) {
      return "DCA with this file name already exists.";
    }
    if (checkIfPositiveInteger(investmentAmount, "Quit") != 1) {
      return "We only accept positive whole numbers for investment amount.";
    }
    if (!checkDateFormat(startingDate)) {
      return "Wrong date format for start date.";
    }
    if (!checkDateFormat(endingDate)) {
      if (!endingDate.equalsIgnoreCase("none")) {
        return "Wrong date format for end date.";
      }
    }
    if (checkDateBeforeCreatedDate(pName, startingDate)) {
      return "Start date must be after portfolio creation date.";
    }
    if (!endingDate.equalsIgnoreCase("none")
            && !LocalDate.parse(endingDate).isAfter(LocalDate.parse(startingDate))) {
      return "End date must be before or on start date.";
    }
    if (checkIfPositiveInteger(recurringDays, "Quit") != 1) {
      return "Frequency must be in days (positive Integer value)";
    }
    return "ok";
  }
}
