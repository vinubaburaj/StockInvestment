We have implemented a flexible and an inflexible portfolio in this assignment along with a GUI to access the same.
...
11) User can create an investment strategy to periodically invest an amount into stocks specified by them. Currently we are only supporting a Dollar Cost Average(DCA) investment strategy. However, there is provision to include more such strategies in the future.
12) We allow the user to add multiple DCAs for a single portfolio.
13) The user can add DCA for one particular day, or for a period of time. A DCA with an end date 'none' lets the user to invest on the portfolio indefinitely in the future.
14) The application will be able to support new APIs that might be required in the future.
15) We have provided the user with an option to work with GUI, which supports the same functionalities as CLI for a flexible portfolio, if they opt to do so. 
