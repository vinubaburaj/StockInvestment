Design changes made in Stocks(Part 3):

1. Changed the method signature pass a Double type value and read a Double type value to/from the XML. 
This was done to incorporate the changes of an investment strategy feature, which could possibly enter decimal value of shares based on the amount and weightage specified by the user. 
The user is however allowed to enter only integer value of shares manually.

Additional design notes in Stocks(Part 3):

1. Created new methods in FlexiblePortfolioInterface to help support the functionalities needed by an investment strategy. 
Any new investment strategy can be added to the program by extending from the PortfolioFlexible class(similar to how dollar cost averaging is implemented)

2. Implemented a GUI to work with flexible portfolios using JFrame. Created an Interface with the basic functionality that a GUI view should support so that different GUIs can implement from this Interface.

3. Created a new controller interface to work with the GUI. This interface contains the basic functionality that a controller working with a GUI should support. 
The implementation of this controller can be utilized in the future to work with other GUIs.
